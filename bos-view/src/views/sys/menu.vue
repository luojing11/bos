  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="创建时间" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.url | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <!--     自定义列-->
...
<script>
  //
  import { add, update, list, deleteUser } from '@/api/sys/menu'
    ...
    methods: {
      ...
      createData() {
        ...
        this.$refs['dataForm'].validate((valid) => {
          ...
          if (valid) {
            ...
            // https.fetchPost("http://localhost/bos/user/list",this.listQuery).then((response) => {
            //   // 关闭对话框
            //   this.dialogFormVisible = false
            //   // 显示一个通知
            //   this.$notify({
      ...
      handleDelete(row) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          https.fetchPost("http://localhost/bos/user/deleteUser",row).then((response) => {
            this.$notify({
              title: '成功',
              message: '删除成功',
              type: 'success',
