<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="上级机构" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.parentStr}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible">
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="部门" prop="dept">
          <el-select v-model="temp.parentStr" placeholder="请选择">
            <el-option-group
...
<script>
  //
  import { add, update, list, deleteUser } from '@/api/sys/dept'
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          //dept: [{type:'number' ,required:true, message: '部门必须选', trigger: 'change' }],
    ...
    methods: {
      ...
      handleDelete(row) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          https.fetchPost("http://localhost/bos/dept/deleteDept",row).then((response) => {
            this.$notify({
              title:"成功",
              message: response.data.message,
              type: 'success',
